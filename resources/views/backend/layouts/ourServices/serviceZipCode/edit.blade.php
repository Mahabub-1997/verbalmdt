        ...
        <div class="content">
            <div class="container-fluid">
                ...
                <div class="card shadow-sm">
                    <div class="card-body">
                        <form action="{{ route('zip_codes.update', $zipCode->id) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="form-group mb-3">
                                <label for="code">Zip Code <span class="text-danger">*</span></label>
                                <input type="text" name="code" class="form-control" value="{{ old('code', $zipCode->code) }}" required>
                            </div>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="parish_id" class="form-control" required>
                                    ...
                                    @foreach($parishes as $parish)
                                        <option value="{{ $parish->id }}" {{ old('parish_id', $zipCode->parish_id) == $parish->id ? 'selected' : '' }}>
                                            {{ $parish->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <button type="submit" class="btn bg-gradient-teal">
                                <i class="fas fa-sync-alt"></i> Update Zip Code
                            </button>
                        </form>
                    </div>
                </div>
